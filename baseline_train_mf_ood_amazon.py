...
def run_a_trail(train_config,log_file=None, save_mode=False,save_file=None,need_train=True,warm_or_cold=None):
    ...
    data_dir = "/home/minseong/CoLLM/collm-datasets/book/"
    train_data = pd.read_pickle(data_dir+"train_ood2_genres_vicuna7b_normalized.pkl")[['uid','iid','label']].values
    valid_data = pd.read_pickle(data_dir+"valid_ood2_genres_vicuna7b_normalized.pkl")[['uid','iid','label']].values
    ...
    test_data = pd.read_pickle(data_dir+"test_ood2_genres_vicuna7b_normalized.pkl")[['uid','iid','label']].values
    ...
    if warm_or_cold is not None:
        if warm_or_cold == 'warm':
            test_data = pd.read_pickle(data_dir+"test_ood2_genres_vicuna7b_normalized.pkl")[['uid','iid','label', 'not_cold']]
...
### Stage 1 : search best hyper-parameters
...
#     embedding_size_ = [256]
#     try:
#         f = open("book_parameters"+str(lr_[0])+".log",'rw+')
#     except:
#         f = open("book_parameters"+str(lr_[0])+".log",'w+')
#     for lr in lr_:
#         for wd in dw_:
...
if __name__=='__main__':
    ...
    dw_ = [1e-6]
    embedding_size_ = [256]
    save_path = "/home/minseong/CoLLM/PretrainedModels/mf/"
    f=None
    for lr in lr_:
        for wd in dw_:
            for embedding_size in embedding_size_:
                ...
                print(train_config)
                save_path += "0715_book_oodv2_best_model_d" + str(embedding_size)+ 'lr-'+ str(lr) + "wd"+str(wd) + ".pth"
...
#                 }
#                 print(train_config)
#                 save_path = "/home/minseong/CoLLM/PretrainedModels/mf/0715_book_oodv2_best_model_d256lr-0.001wd1e-06.pth"
                
#                 run_a_trail(train_config=train_config, log_file=f, save_mode=False,save_file=save_path,need_train=False,warm_or_cold=None)
#     if f is not None:
#         f.close()
