...
class MiniGPT4Rec_v2(Rec2Base):
    ...
    def __init__(
        ...
        # self.llama_tokenizer.pad_token = self.llama_tokenizer.eos_token # MODIFIED
        ...
        self.genre_names = sorted(GENRE_PRIORITY)
        
        self.genres = [f"[GENRE_{name.upper().replace(' ', '_')}]" for name in self.genre_names]
        self.id_placeholder_token = "[ID_EMB_PLACEHOLDER]"
        special_tokens_to_add = self.genres + [self.id_placeholder_token]
        num_added_toks = self.llama_tokenizer.add_tokens(special_tokens_to_add, special_tokens=True)
        print(f"Added {num_added_toks-1} new genre tokens to the tokenizer.")

        if self.llama_tokenizer.pad_token is None:
            self.llama_tokenizer.add_special_tokens({'pad_token': '[PAD]'})

        self.id_placeholder_token_id = self.llama_tokenizer.convert_tokens_to_ids(self.id_placeholder_token)
        if self.id_placeholder_token_id == self.llama_tokenizer.unk_token_id:
            logging.error(f"CRITICAL: ID Placeholder token '{self.id_placeholder_token}' was resolved to UNK token!")

        self.genre_to_idx = {genre: i for i, genre in enumerate(self.genres)}
        self._genre_token_ids_tensor = None
        ### ===== MODIFIED (END) ===== ###
        ...
        self.llama_model.resize_token_embeddings(len(self.llama_tokenizer)) # MODIFIED
    ...
    def print_prompt(self):
        print('Prompt Pos Example \n{} {}, {} or {}'.format(random.choice(self.prompt_list),random.choice(self.genre_names),self.pos_ans[0],self.neg_ans[0]))
    ...
    def recprompt_wrap_v2(self, samples, ori_samples, atts_sample, prompt): # used for stage 2
        if prompt:
            ...
            id_placeholder_str = self.id_placeholder_token
            prompt_template_with_placeholders = bos + prompt_ori
            if "<UserID>" in prompt_ori:
                prompt_template_with_placeholders = prompt_template_with_placeholders.replace("<UserID>", id_placeholder_str)
            if "<TargetItemID>" in prompt_ori:
                prompt_template_with_placeholders = prompt_template_with_placeholders.replace("<TargetItemID>", id_placeholder_str)
            if "<DCNFeature>" in prompt_ori:
                prompt_template_with_placeholders = prompt_template_with_placeholders.replace("<DCNFeature>", id_placeholder_str)
            ...
            for k in range(batch_size):
                current_prompt = prompt_template_with_placeholders

                if 'InteractedItemTitles' in ori_samples and 'InteractedNum' in ori_samples:
                    current_prompt = current_prompt.replace("<ItemTitleList>", ori_samples['InteractedItemTitles'][k])
                if 'TargetItemTitle' in ori_samples:
                    current_prompt = current_prompt.replace("<TargetItemTitle>", ori_samples['TargetItemTitle'][k])
                if 'InteractedNum' in ori_samples and '<ItemIDList>' in current_prompt:
                    current_prompt = current_prompt.replace('<ItemIDList>', ', '.join([id_placeholder_str] * ori_samples['InteractedNum'][k]))
            ...
            id_placeholder_locations = torch.nonzero(prompts_tokens.input_ids == self.id_placeholder_token_id)
            if not self.has_pri_decode:
                print("#######prmpt decoded example: ",' '.join(self.llama_tokenizer.batch_decode(prompts_tokens.input_ids[0])))
                self.has_pri_decode = True
            ...
            if "<UserID>" in prompt_ori  and "<ItemIDList>" in prompt_ori and  "<TargetItemID>" in prompt_ori:
                prompt_embeds[id_placeholder_locations[:,0],id_placeholder_locations[:,1]] = samples['merged_embs']
            elif "<UserID>" in prompt_ori and "<TargetItemID>" in prompt_ori and "<ItemIDList>" not in prompt_ori:
                prompt_embeds[id_placeholder_locations[:,0],id_placeholder_locations[:,1]] = torch.cat([samples['User_emb'], samples['TargetItem_emb']],dim=-2).reshape(-1,samples['User_emb'].shape[-1])
            elif "<DCNFeature>" in prompt_ori:
                prompt_embeds[id_placeholder_locations[:,0],id_placeholder_locations[:,1]] = samples['User_emb'].reshape(-1,samples['User_emb'].shape[-1])
            else:
                pass 
            return prompt_embeds, prompts_tokens.attention_mask
    ...
    def _ensure_genre_token_ids_initialized(self, device):
        if self._genre_token_ids_tensor is None:
            print("Initializing genre token IDs...")
            genre_token_ids_list = []
            
            print("===== Genre Tokenization Details =====")

            for i, genre_name in enumerate(self.genres):
                current_token_id = self.llama_tokenizer.convert_tokens_to_ids(genre_name)
                
                if current_token_id == self.llama_tokenizer.unk_token_id:
                    print(f"CRITICAL WARNING: Special token '{genre_name}' resolved to UNK token. Check tokenizer setup!")
            
                genre_token_ids_list.append(current_token_id)

                decoded_str_from_id = self.llama_tokenizer.decode([current_token_id])
                standard_name = self.genre_names[i]
                print(f"Standard Name: '{standard_name}' -> Special Token: '{genre_name}' -> Selected ID: {current_token_id} -> Decoded from ID: '{decoded_str_from_id}'")
            
            print("====================================")

            self._genre_token_ids_tensor = torch.tensor(genre_token_ids_list, dtype=torch.long).to(device)
            print(f"Genre token on device {device}: {self._genre_token_ids_tensor.tolist()}")
        
    def custom_bce_loss(self, predictions, targets, epsilon=1e-7):
    ...
    def forward_v2(self, samples):
        ...
        rec_loss = nn.functional.binary_cross_entropy_with_logits(logits, samples['label'].float()) # MODIFIED

        ### ===== MODIFIED ===== ###
        # logits_yesno_ = outputs.logits[:,-t_posi,:][:,[pos_ans_id, neg_ans_id]]
        # prob_yesno = F.softmax(logits_yesno_, dim=1)
        # prob_pos = prob_yesno[:, 0]
        # rec_loss = self.custom_bce_loss(prob_pos, samples['label'].float())

        genre_loss = torch.tensor(0.0, device=device)
        if 'likedgenres_indices' in samples:
            self._ensure_genre_token_ids_initialized(device)

            all_token_logits_at_pos = outputs.logits[:, -t_posi, :]
            genre_specific_logits = all_token_logits_at_pos.index_select(dim=1, index=self._genre_token_ids_tensor)

            ### ===== ENTROPY ===== ###
            with torch.no_grad():
                probs = torch.sigmoid(genre_specific_logits)
                entropy = -(probs * torch.log(probs + self.nozero) + (1 - probs) * torch.log(1 - probs + self.nozero))
                avg_entropy = entropy.mean(dim=1)
                entropy_score = torch.sqrt(torch.mean(avg_entropy.pow(2)))
            ### ===== ENTROPY ===== ###

            liked_indices = samples['likedgenres_indices'].to(device)
            batch_size = liked_indices.shape[0]
            num_genres = len(self.genres)

            genre_target = torch.zeros(batch_size, num_genres, device=device)
            for i in range(batch_size):
                valid_indices = liked_indices[i][liked_indices[i] != -100]
                if len(valid_indices) > 0:
                    genre_target[i, valid_indices] = 1.0

            # if random.random() < 0.01:
            #     sample_liked_indices = samples['likedgenres_indices'][0]
            #     valid_indices = sample_liked_indices[sample_liked_indices != -100]
            #     ground_truth_genre_names = [self.genre_names[i] for i in valid_indices.tolist()]
            #     ground_truth_str = ", ".join(ground_truth_genre_names)
            #     logits_for_first_sample = genre_specific_logits[0]
            #     sorted_logits_for_sample = sorted(zip(self.genre_names, logits_for_first_sample.tolist()), key=lambda x: x[1], reverse=True)
            #     print("\n---------- Debugging ----------")
            #     print(f"[Debug] entropy_score: {entropy_score.item():.4f}")
            #     print(f"    - (Sample Ground Truth): {ground_truth_str}")
            #     print("    - (Logits per Genre):")
            #     for name, logit in sorted_logits_for_sample:
            #         print(f"        - {name:<20}: {logit: .4f}")
            #     print("-------------------------------\n")

            criterion_genre = nn.BCEWithLogitsLoss()
            genre_loss = criterion_genre(genre_specific_logits, genre_target)
        

        weight = entropy_score
        # weight = 1.0
        total_loss = rec_loss + weight * genre_loss

        if torch.isnan(total_loss) or torch.isinf(total_loss):
             print("NaN or Inf detected in total_loss!")
             print(f"rec_loss: {rec_loss.item()}, genre_loss: {genre_loss.item() if isinstance(genre_loss, torch.Tensor) else genre_loss}")
        ### ===== MODIFIED (END) ===== ###

        # return {"loss": loss}
        return {"loss": total_loss, "rec_loss": rec_loss, "genre_loss": genre_loss, "weight": weight} # MODIFIED
...
#                 }
    ...
    def generate_for_samples_v2(self, samples,return_all=False):
        ...
        rec_loss = nn.functional.binary_cross_entropy_with_logits(logits_, samples['label'].float()) # MODIFIED
        ...
        if return_all:
            ...
            return {"outputs": outputs, "logits": logits_, "loss": total_loss, "rec_loss": rec_loss, "genre_loss": genre_loss, "weight": weight} # MODIFIED

        # return {"loss": loss, 'logits':logits_}
        return {"loss": total_loss, "rec_loss": rec_loss, "genre_loss": genre_loss, "weight": weight, 'logits': logits_} # MODIFIED
