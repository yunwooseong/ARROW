...
class RecBaseTask(BaseTask):
    ...
    def evaluation(self, model, data_loaders, cuda_enabled=True):
        ...
        metric_logger.add_meter("genre_loss", SmoothedValue(window_size=1, fmt="{value:.4f}"))
        metric_logger.add_meter("weight", SmoothedValue(window_size=1, fmt="{value:.4f}"))
        metric_logger.add_meter("acc", SmoothedValue(window_size=1, fmt="{value:.4f}"))
        auc_logger.add_meter("auc", SmoothedValue(window_size=1, fmt="{value:.4f}"))
        header = "Evaluation"
        # TODO make it configurable
        ...
        for data_loader in data_loaders.loaders:
            ...
            for samples in metric_logger.log_every(data_loader, print_freq, header):
                # samples = next(data_loader)
                samples = prepare_sample(samples, cuda_enabled=cuda_enabled)
                eval_output_dict = self.valid_step(model=model, samples=samples)
                total_loss_eval = eval_output_dict["loss"]
                rec_loss_eval = eval_output_dict.get("rec_loss", torch.tensor(0.0))
                genre_loss_eval = eval_output_dict.get("genre_loss", torch.tensor(0.0))
                weight_eval = eval_output_dict.get("weight", torch.tensor(0.0))
                # results_loss.append(eval_output['loss'].item())
                if 'logits' in eval_output_dict.keys():
                    use_auc = True
                    users.extend(samples['UserID'].detach().cpu().numpy())
                    results_logits.extend(eval_output_dict['logits'].detach().cpu().numpy())
                    labels.extend(samples['label'].detach().cpu().numpy())
                    logits = eval_output_dict['logits']
                    logits[logits>0.5] = 1
                    acc = (logits-samples['label'])
                    acc = (acc==0).sum()/acc.shape[0]
                    metric_logger.update(acc=acc.item())
                ...
                # acc = accuracy_score(samples['label'].cpu().numpy().astype(int), logits.astype(int))
                # results.extend(eval_output)
                metric_logger.update(loss=total_loss_eval.item())
                metric_logger.update(rec_loss=rec_loss_eval.item() if isinstance(rec_loss_eval, torch.Tensor) else rec_loss_eval)
                metric_logger.update(genre_loss=genre_loss_eval.item() if isinstance(genre_loss_eval, torch.Tensor) else genre_loss_eval)
                metric_logger.update(weight=weight_eval.item() if isinstance(weight_eval, torch.Tensor) else weight_eval)
                torch.cuda.empty_cache()
            results_logits_ = torch.tensor(results_logits).to(eval_output_dict['logits'].device).contiguous()
            labels_ = torch.tensor(labels).to(eval_output_dict['logits'].device).contiguous()
            users_ = torch.tensor(users).to(eval_output_dict['logits'].device).contiguous()
            # if use_auc:
